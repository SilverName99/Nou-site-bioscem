(() => {
    ...
    // Bulina de chat stă peste sertar și are un z-index pe care nu-l putem
    // depăși cinstit din CSS. O stinge helper-ul comun din layout, care ține
    // socoteala motivelor — coșul ăsta e doar unul dintre ele, meniul de pe
    // telefon e altul, iar închiderea unuia nu trebuie să reaprindă bulina
    // peste celălalt.
    const potrivesteChatul = (open) => {
        if (open) {
            window.bvChatWidget?.ascunde("cos-flotant");
        } else {
            window.bvChatWidget?.arata("cos-flotant");
        }
    };
